...
import static org.assertj.core.api.Assertions.assertThat;
    ...
    void test4(long id) {
        ...
        // then
        assertThat(result).isGreaterThan(0); // 작성한게 0보다 큰지
        Board boardUpdated = boardDao.findById(session, id); // id로 검색
        assertThat(boardUpdated).satisfies((b) -> {
            assertThat(b.getTitle()).isEqualTo(newTitle); // 새 제목이 맞는지
            assertThat(b.getContent()).isEqualTo(newContent); // 새 내용이 맞는지
        });
    }
    @DisplayName("게시글 삭제")
    @ParameterizedTest
    ...
    void test5(long id) {
        ...
        // then
        assertThat(result).isGreaterThan(0);
        Board boardDeleted = boardDao.findById(session, id); // id로 검색
        assertThat(boardDeleted).isNull(); // null이 맞는지
    }
...
//       IntStream.range -> list나 배열을 제어
        }
    /* Provider는 static으로 작성 */

        /**
         * test(int, String, boolean)
         *
         * Stream<Arguments> -> 인자가 여러개일때 사용하면 좋음
         * - Arguments.arguments(1, "hello", true)
         * - Arguments.arguments(1, "bye", false)
         * - Arguments.arguments(1, "ㅋㅋ", true)
         *
         * @return
         */
