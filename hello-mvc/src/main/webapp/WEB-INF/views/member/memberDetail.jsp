        ...
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form name="memberUpdateFrm" action="${pageContext.request.contextPath}/member/memberUpdate" method="post" class="space-y-4 md:space-y-6">
                ...
                <fieldset>
                    ...
                    <%-- c:set태그는 context객체별 속성을 생성하는것(c:remove(속성지우기)와 반대 / scope="page"는 이번페이지에서 가능 --%>
...
<%--                        <c:if test="${hobby != '운동' && hobby != ' 게임' && hobby != ' 영화' && hobby != ' 음악'}"> --%>
<%--                        <c:if test="${hobby ne '운동' && hobby ne ' 게임' && hobby ne ' 영화' && hobby ne ' 음악'}"> --%>
                        <c:if test="${not hobbyDefaultList.contains(hobby)}" >
                            <div class="inline-flex items-center mr-4">
                                <input id="hobby-${vs.count + 4}" type="checkbox" name="hobby" value="${hobby}" checked class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2" ${loginMember.hobby.contains('음악') ? 'checked' : ''} >
                ...
                <p class="text-sm font-light text-red-500">
                    <%-- a태그는 get방식이라 우회해주려고 사용 -> deleteMmber();, document.memberDeleteFrm.submit(); --%>
                    <%-- 탈퇴나 삭제시 한번 더 물어봐주기 -> confirm('정말 탈퇴하시겠습니까?😤) --%>
